...
def submit_event(event=None):
    ...
    if not this.testing:
        url="https://api.canonn.tech/grreports"
        payload = {
            "userType": "pc",
            "reportType": "new",
            "reportStatus": "pending",
            "type": this.types.current().title(),
            "systemAddress": this.entry.get("SystemAddress"),
            "systemName": this.system,
            "bodyName": this.entry.get("BodyName"),
            "latitude": this.entry.get("Latitude"),
            "longitude": this.entry.get("Longitude"),
            "reportComment": json.dumps(this.entry, indent=4),
            "frontierID": get_index(this.entry.get("Name")),
            "cmdrName": this.cmdr,
            "isBeta": this.is_beta,
...
def journal_entry(cmdr, is_beta, system, station, entry, state):
    rtest = (entry.get("event") == "SendText" and entry.get(
        "Message") and "test ruin scanner" in entry.get("Message"))

    detected=(entry.get("event") == "ApproachSettlement" and "$Ancient:#index=" in entry.get("Name"))
